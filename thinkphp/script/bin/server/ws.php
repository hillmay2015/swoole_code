...
//        }
    ...
    {
        //swoole_set_process_name('live_master');//设置主进程的一个别名   mac不支持 使用linux可以支持
    }
...
//        echo  $request->fd;
    ...
    {
        $datas = array_merge(['data' => date('Y-m-d H:i:s')], $_POST, $_GET, $_SERVER);
        ...
        //  echo APP_PATH . '../runtime/log/' . date('Ym') . '/' . date('d') . '/access.log';
...
//20台 机器    agent->spark  日志解析(计算) ->数量存入分布式数据库es   > hadoop
//php 去拿数据


//sgiterm 停止服务器
// sigurs1 重启worker进程
//sigurs2   重启task worker进程